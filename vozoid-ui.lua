...
function utility.rotatevector2(r)
    return Vector2.new(math.sin(math.rad(r)), math.cos(math.rad(r)));
end
...
local decode = base64_decode or base64decode or (syn and syn.crypt.base64.decode) or (crypt and crypt.base64decode or (crypt and crypt.base64 and crypt.base64.decode))
...
function library:Close()
    ...
    if self.cursor then
        self.cursor[1].Visible = self.open
        self.cursor[2].Visible = self.open
    end
end
...
function library:Load(options)
    ...
    local cursoroutline = utility.create("Triangle", {
        Thickness = 1,
        Filled = false,
        Color = Color3.new(),
        ZIndex = 1001
